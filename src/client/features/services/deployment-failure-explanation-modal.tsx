...
import { api, type Deployment, type DeploymentFailureExplanation } from "../../api";
...
export function DeploymentFailureExplanationModal({
  ...
  useEffect(() => {
    ...
    void api.explainDeploymentFailure(deployment.id)
      .then((response) => {
  ...
  return (
    ...
    >
      {loading ? (
        <div className="border border-zinc-800 bg-zinc-900/55 px-4 py-3 font-mono text-[10px] uppercase tracking-[0.18em] text-zinc-500">
            ...
            <section className="border border-zinc-800 bg-zinc-950/45 p-4">
              ...
              <div className="mt-3 space-y-2">
                {explanation.commands.map((command, index) => (
                  <pre key={`${command}-${index}`} className="overflow-x-auto border border-zinc-800 bg-zinc-900/70 px-3 py-2 font-mono text-xs text-zinc-200">
                    {command}
                  </pre>
                ))}
              </div>
