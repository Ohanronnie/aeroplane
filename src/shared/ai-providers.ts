...
export const aiProviderIdValues = ["openai", "anthropic", "google", "mistral", "groq", "deepseek", "xai", "moonshot"] as const;
...
export function isAiProviderModel(providerId: AiProviderId, modelId: unknown): modelId is string {
  return typeof modelId === "string" && aiProviderModels(providerId).some((model) => model.id === modelId);
}
